...
export default function DraftTemplatesPage() {
  ...
  const refresh = async () => {
    setLoading(true);
    try {
      setDrafts(await listDrafts("template"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold">Draft Templates</h1>
        <p className="text-sm text-zinc-400">
          Import từ Kế hoạch mẫu rồi chỉnh sửa tại đây trước khi chuyển thành Nhiệm vụ thật.
        </p>
